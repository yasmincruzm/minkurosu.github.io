...
let current_theme;
try {
	const requested_theme = new URLSearchParams(location.search).get("theme");
	if (requested_theme === "retro-dark.css") {
		// An embed can request the built-in dark skin without affecting the
		// visitor's saved theme preference in a standalone JS Paint session.
