...
const PAGE_WRAP = "p-4 sm:p-6 md:p-8 flex flex-col items-center min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-inter rounded-lg shadow-lg";
const PAGE_TITLE = "text-4xl sm:text-5xl font-bold mb-8 text-center";
...
function BlogList({ userId, blogPosts, message, setMessage }) {
    return (
        <div className={PAGE_WRAP}>
            <h1 className={PAGE_TITLE}>Meus Posts do Blog</h1>
...
function NewPost({ userId, db, message, setMessage, setCurrentView }) {
    ...
    return (
        <div className={PAGE_WRAP}>
            <h1 className={PAGE_TITLE}>Escrever Novo Post do Blog</h1>
            <MessageBanner message={message} onClose={() => setMessage('')} />
...
function NavButton({ active, onClick, children }) {
    ...
    return (
        <button onClick={onClick} className={`py-2 px-4 rounded-md text-lg font-medium transition duration-300 ease-in-out ${activeClasses}`}>
            {children}
        </button>
    );
}
        ...
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900">
            <header className="bg-white dark:bg-gray-800 shadow-md p-4 flex justify-center sticky top-0 z-10 rounded-b-lg">
                <nav className="flex space-x-4">
                    <NavButton active={currentView === 'list'} onClick={() => setCurrentView('list')}>
                        Ver Posts do Blog
                    </NavButton>
                    <NavButton active={currentView === 'newPost'} onClick={() => setCurrentView('newPost')}>
                        Escrever Novo Post
                    </NavButton>
                </nav>
            </header>
            ...
            <main className="container mx-auto p-4">
                {currentView === 'newPost'
                    ? <NewPost db={db} userId={userId} message={message} setMessage={setMessage} setCurrentView={setCurrentView} />
                    : <BlogList userId={userId} blogPosts={blogPosts} message={message} setMessage={setMessage} />}
            </main>
        </div>
    );
};
